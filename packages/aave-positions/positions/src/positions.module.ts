 ...
 * here — the one table that was fetched rather than folded. That and reserve
 * prices are now `@aave-positions/enrichment`, which is where the shape they
 * share belongs: small keyed dimensions, replaced in place, filled by a worker
 * rather than by a view. What is left here is the fold, and the fold is
 * ClickHouse.
 *
 * ```ts
