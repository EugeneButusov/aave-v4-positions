-- Dropped so `position_liquidation_debt` can be recreated against a
-- `user_positions` that carries the instant each delta happened at. Recreated
-- at `076` under the same name, reading the same events.
--
...
-- the backfill at `084` exact rather than double-counted.
DROP VIEW IF EXISTS position_liquidation_debt;
