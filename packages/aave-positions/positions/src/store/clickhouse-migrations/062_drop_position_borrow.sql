-- Dropped so `position_borrow` can be recreated against a
-- `user_positions` that carries the instant each delta happened at. Recreated
-- at `072` under the same name, reading the same events.
--
...
-- the backfill at `080` exact rather than double-counted.
DROP VIEW IF EXISTS position_borrow;
