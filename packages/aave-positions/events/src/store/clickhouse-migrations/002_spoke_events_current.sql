-- Every read goes through this, never the raw table.
--
-- Not a convenience. FINAL does *not* filter an unpaired retraction row —
-- measured: a lone sign = -1 shows up under FINAL and is absent here — so a
-- consumer reading the table directly would see retractions as though they
-- were events.
--
...
CREATE VIEW IF NOT EXISTS spoke_events_current AS
SELECT
    chain_id,
    block_number,
    log_index,
