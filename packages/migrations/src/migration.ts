...
/**
 * One `.sql` file, and the statement in it.
 *
 ...
 * surrounding code can quietly alter. One statement per file, so a file is
 * sent as it stands and nothing has to take it apart.
 *
...
export interface Migration {
  ...
  readonly id: string;
  /**
   * The file, verbatim — leading prose and trailing `;` included, both of which
   * every server here accepts. Holding two would fail on ClickHouse, whose HTTP
   * interface refuses a multi-statement body, and that is the whole enforcement.
   */
...
async function readDirectory(directory: string): Promise<Migration[]> {
  ...
  return Promise.all(
    files.map(async (file) => ({
      ...
      statement: await readFile(join(directory, file), 'utf8'),
    })),
  );
}
