...
export default defineConfig({
  ...
  test: {
    ...
    env: {
      NODE_ENV: 'test',
      LOG_LEVEL: 'silent',
      // The listing source reads the Hub fold, so its spec runs against a real
      // ClickHouse; the store is Postgres. Nothing is mocked — what these prove
      // is that the SQL finds what it claims to, and no double can say that.
